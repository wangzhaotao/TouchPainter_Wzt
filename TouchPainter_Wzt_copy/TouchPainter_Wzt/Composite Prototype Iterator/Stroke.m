...
- (CGPoint) location
{
  ...
  {
      id <Mark> obj = [children_ objectAtIndex:0];
    return [obj location];
  }
...
- (id)initWithCoder:(NSCoder *)coder
{
    if (self = [super init])
    {
        color_ = [coder decodeObjectForKey:@"StrokeColor"];// retain]; //tyler
...
{
    return [[MarkEnumerator alloc] initWithMark:self];// autorelease]; //tyler
}
